...
Ccsv.foreach(ARGV[0]) do |values|
  ...
  fromuri = values[0].chomp#.gsub("support.mozilla.org", "support-stage.allizom.org") # column A
  from_uri= URI(fromuri)
  $stderr.puts(values[5])
  $stderr.puts(values[0])

  touri = values[2].chomp#.gsub("support.mozilla.org", "support-stage.allizom.org") #column C
  to_uri = URI(touri)
  try_count = 0
  begin 
